...
def main(cfg):
    ...
    def model_predict(inputs):
        z1_dim = 225
        # inputs: a numpy array with concatenated y2 and z1
        y2, z1 = inputs[:, :-z1_dim], inputs[:, -z1_dim:]

        # Convert to torch tensors
        y2_tensor = torch.tensor(y2, dtype=torch.float32).to(device)
        z1_tensor = torch.tensor(z1, dtype=torch.float32).to(device)

        # Perform model inference
        with torch.no_grad():
            output = model.classifier(torch.cat((y2_tensor, z1_tensor), dim=1))
    ...
    # Register hooks
    y2_hook = model.image_feature_extractor.register_forward_hook(hook_fn)
    z1_hook = model.clinical_model.register_forward_hook(hook_fn)

    y2_dim = 4096
    z1_dim = 225

    # sample_y2 = np.random.rand(1, y2_dim)  # Replace y2_dim with actual dim
    # sample_z1 = np.random.rand(1, z1_dim)  # Replace z1_dim with actual dim
    # sample_data = np.concatenate((sample_y2, sample_z1), axis=1)
    # explainer = shap.KernelExplainer(model_predict, sample_data)

    # shap_values = explainer.shap_values(sample_data)
    # shap_y2 = shap_values[:, :y2_dim]
    # shap_z1 = shap_values[:, -z1_dim:]

    # # Aggregate the contributions
    # y2_contribution = np.sum(np.abs(shap_y2), axis=1)  # Per sample
    # z1_contribution = np.sum(np.abs(shap_z1), axis=1)  # Per sample

    # Compare contributions
    # for i in range(len(sample_data)):
    #     print(f"Sample {i}: y2 contribution={y2_contribution[i]}, z1 contribution={z1_contribution[i]}")

    # test the model with validation set
    with torch.no_grad():
        model.eval()
        y2_buffer = np.zeros((10, 4096))
        z1_buffer = np.zeros((10, 225))
        targets = []
        preds = []
        patients = []
        ...
        for k, data in enumerate(test_loader):
            ...
            y2_buffer[k] = intermediate_outputs[model.image_feature_extractor].cpu().numpy()
            z1_buffer[k] = intermediate_outputs[model.clinical_model].cpu().numpy()
    ...
    data = np.concatenate((y2_buffer, z1_buffer), axis=1)
    ...
    # Aggregate the contributions
    y2_contributions = np.sum((shap_y2), axis=1)  # Per sample
    z1_contributions = np.sum((shap_z1), axis=1)  # Per sample

    # Compare contributions
    for i in range(len(data)):
        y2_contribution = np.abs(y2_contributions[i].item())
        z1_contribution = np.abs(z1_contributions[i].item())
        img_contribution = y2_contribution / (y2_contribution + z1_contribution)
        clinical_contribution = z1_contribution / (y2_contribution + z1_contribution)
        print(f"Sample {i}: y2 contribution={y2_contribution:.4f}, z1 contribution={z1_contribution:.4f}")

        print(
            f"Sample {i}: img contribution={img_contribution.item():.4f}, clinical info contribution={clinical_contribution.item():.4f}"
        )
