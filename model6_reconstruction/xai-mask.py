# Use gradient based method to identify which modal is important for the prediction
...
def main(cfg):
    ...
    for k, data in enumerate(test_loader):
        ...
        # zero out the proc_data["img"]
        proc_data["img"] = torch.zeros_like(proc_data["img"])

        model.zero_grad()
        pred.backward()

        with torch.no_grad():
            full_modality_prediction = F.sigmoid(
                model(proc_data["CT_image"].float(), proc_data["img"].float(), clinical_data.float())
            )
            no_modality_prediction = F.sigmoid(
                model(
                    torch.zeros_like(proc_data["CT_image"]).float(),
                    torch.zeros_like(proc_data["img"]).float(),
                    torch.zeros_like(clinical_data).float(),
                )
            )
            CT_img_pred = F.sigmoid(
                model(proc_data["CT_image"].float(), proc_data["img"].float(), torch.zeros_like(clinical_data).float())
            )
            CT_clinical_pred = F.sigmoid(
                model(proc_data["CT_image"].float(), torch.zeros_like(proc_data["img"]).float(), clinical_data.float())
            )
            img_clinical_pred = F.sigmoid(
                model(torch.zeros_like(proc_data["CT_image"]).float(), proc_data["img"].float(), clinical_data.float())
            )

            CT_only_pred = F.sigmoid(
                model(
                    proc_data["CT_image"].float(),
                    torch.zeros_like(proc_data["img"]).float(),
                    torch.zeros_like(clinical_data).float(),
                )
            )
            img_only_pred = F.sigmoid(
                model(
                    torch.zeros_like(proc_data["CT_image"]).float(),
                    proc_data["img"].float(),
                    torch.zeros_like(clinical_data).float(),
        ...
        # Compute the importance of each modality
        image_modality_importance = full_modality_prediction - CT_clinical_pred
        CT_modality_importance = full_modality_prediction - img_clinical_pred
        clinical_modality_importance = full_modality_prediction - CT_img_pred

        # Compute the importance of each modality
        image_modality_contribution = img_only_pred - no_modality_prediction
        CT_modality_contribution = CT_only_pred - no_modality_prediction
        clinical_modality_contribution = clinical_only_pred - no_modality_prediction

        print(f"Patient ID: {patient_id}")
        print(f"{image_modality_importance=}")
        print(f"{CT_modality_importance=}")
        print(f"{clinical_modality_importance=}")

        print(f"{image_modality_contribution=}")
        print(f"{CT_modality_contribution=}")
        print(f"{clinical_modality_contribution=}")
        ...
        plt.imsave(f"heatmap_{patient_id}_dec_attn1.png", attn_heatmap)
