...
def main(cfg):
    ...
    for k, data in enumerate(test_loader):
        ...
        shap_values = explainer.shap_values(proc_data["img"].to(device).to(torch.float32), nsamples=200)

        from PIL import Image
        shap_img = shap_values.squeeze()
        image = Image.fromarray(np.transpose(shap_img, (1,2,0)))
        image.save(f"shap_{k}.png")
