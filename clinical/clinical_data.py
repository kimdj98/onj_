...
def data_make(data):
    ...
    data_x1 = data_x.drop(columns=["pt_CODE"])
    data_x1 = data_x1.astype(dtype=np.float64)

    ###### PO, IV, Dont know ######
    index = data_x1[(data_x1["PMH_PO"] == 9999) & (data_x1["PMH_IVSC"] == 9999)].index
    for i in index:
        data_x1.at[i, "PMH_PO"] = 0
        data_x1.at[i, "PMH_IVSC"] = 0
        data_x1.at[i, "PMH_CK"] = 1
    ...
    ###### change 9999 to NaN ######
    data_x1 = data_x1.replace(to_replace=9999, value=np.NaN)

    ####### replace 8888 to 0 #######
    data_x1 = data_x1.replace(to_replace=8888, value=0)

    ##### delete column with too many nan ######
    # ratio = 0.5
    # data_x1 = data_x1.dropna(thresh=ratio*len(data_x1), axis=1)    # thresh = num of existing data

    ##### delete data too close to label #####
    except_list_2 = ["OE_PA", "OE_SW", "OE_BEX", "OE_PUS", "OE_PAR", "OE_MOB", "OE_IN_FIS", "OE_EX_FIS"]
    data_x1 = data_x1.drop(columns=except_list_2)

    # combine column pt_CODE
    data_x1["pt_CODE"] = data["pt_CODE"]
...
### Preprocess ###
print("Starting preprocessing with 'start' function...")
df = start(df)
print("Data after 'start' preprocessing step.")

print("Shuffling and preparing data with 'data_make' function...")
[data_x, data_y] = data_make(df)
print("Data preparation completed with 'data_make'.")

print("Extracting pt_CODE and dropping the column from data_x...")
pt_CODE = data_x["pt_CODE"]
data_x = data_x.drop(columns="pt_CODE")
print("pt_CODE extracted and data_x updated.")

print("Applying MICE imputation...")  # Bottle neck here: takes a long time
data_x = MICE(data_x)
print("MICE imputation completed.")

print("Merging features with 'merge_f' function...")
data_x = merge_f(data_x)
print("Feature merging completed.")

# Saving the outputs
print("Saving pt_CODE to CSV...")
pt_CODE.to_csv(path + "/pt_CODE.csv")
print("pt_CODE saved.")

print("Saving data_x to CSV...")
data_x.to_csv(path + "/data_X.csv")
print("data_X saved.")

print("Saving data_y to CSV...")
