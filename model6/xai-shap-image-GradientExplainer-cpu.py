...
class Model2DWrapper(nn.Module):
    ...
    def forward(self, x_2d: torch.Tensor):
        ...
        out = self.model(
            self.real_3d.expand(x_2d.shape[0], -1, -1, -1, -1), x_2d, self.real_clinical.expand(x_2d.shape[0], -1)
        )
...
def main(cfg):
    ...
    for k, data in enumerate(test_loader):
        ...
        if k == selected_patient:
            bg_2d = torch.zeros((1, 3, 1024, 1024), dtype=torch.float32).to(device).cpu()
            wrapped_model = Model2DWrapper(model).eval().to(device).cpu()
            wrapped_model.set_real_data(
                proc_data["CT_image"].to(torch.float32).cpu(), clinical_data.to(torch.float32).cpu()
            )
            explainer = shap.GradientExplainer(wrapped_model, bg_2d)
        ...
        pred = model(proc_data["CT_image"].float().cpu(), proc_data["img"].float().cpu(), clinical_data.float().cpu())
        ...
        shap_values = explainer.shap_values(proc_data["img"].to(device).to(torch.float32).cpu(), nsamples=200)

        from PIL import Image

        shap_img = shap_values.squeeze()
        shap_img_uint8 = (shap_img * 255 * 20).astype(np.uint8)
        # Transpose the array to (height, width, channels)
        shap_img_transposed = np.transpose(shap_img_uint8, (1, 2, 0))
        image = Image.fromarray(shap_img_transposed, mode="RGB")
        image.save(f"shap_{k}_{patient_id}.png")
