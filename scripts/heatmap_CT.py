...
from utils.plotting import apply_grad_cam, apply_grad_cam_3d, apply_med_cam
...
def visualize(cfg):
    ...
    transforms = Compose(
        [
            ...
            ScaleIntensityRanged(keys=["CT_image"], a_min=-1000, a_max=2500, b_min=0.0, b_max=1.0, clip=False),
    ...
    for epoch in range(cfg.train.epoch):
        ...
        for i, batch in enumerate(train_iterator):
            ...
            if cfg.mode != "train":  # block training when cfg.mode is not "train"
                ...
                # apply_med_cam(
                #     model.model_2d, target_layer=model.model_2d.classifier_model.conv4, input=batch["PA_image"]
                # )
                apply_med_cam(model.model_3d, target_layer=model.model_3d.layer4[1].conv3, input=batch["CT_image"])
                continue
            assert (
